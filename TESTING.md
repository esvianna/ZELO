...
| 2z2 | **Mapa evento** pinos legíveis (#61, PWA **175+** / plugin **2.23.1+**) | view_ops / ≤768px | Pinos balcão/destino visíveis no zoom completo; toque ≥44 px; seleccionar destino/balcão → zoom no POI; admin «Posicionar» → pinos 2× + linha amarela; F5 persiste coords |
| 2z3 | **Mapa evento** pinos proporcionais (#61, PWA **176+**) | view_ops / ≤768px | «Mapa completo»: pinos escalam com diagrama (sem bolhas gigantes); pinch-in → legíveis até ~26 px; «Ir ao destino» centra POI |
...
| 2z7g | **Export PDF legenda overlay** (#64, plugin **2.24.6+**) | manage_options / `zelo_manage_ops` | Legenda ~44 mm sobreposta à direita da imagem (fundo branco); nomes longos numa linha; números vetoriais e pinos inalterados; overflow → pág. 2 |
| 2z7h | **Export PDF mapa sem corte nas margens** (#64, plugin **2.24.7+**) | manage_options / `zelo_manage_ops` | Mapa ~253 mm centrado (não 281 mm); «Rua Getúlio Vargas» e pinos da direita visíveis; legenda overlay mantida |
| 2z7i | **Export PDF layout 2.24.5 + legenda alargada** (#64, plugin **2.24.8+**) | manage_options / `zelo_manage_ops` | Mapa ~253 mm (como 18:56); legenda coluna ~32 mm (não overlay); «Ambulatório 2/3» numa linha; ruas visíveis; números vetor nos pinos |
| 2z7j | **Export PDF legenda por pavimento + cabeçalho logo** (#66, plugin **2.26.6+**) | manage_options / `zelo_manage_ops` | Título fixo; mapa alinhado à esquerda; overlay com fundo só no texto; **Piso 3 + Sub-solo** na pág. 1 se couber; «LEGENDA (FACILIDADES)» visível; overflow → pág. 2 |
| 2z8 | **Mapa — rótulos portões (kind gate)** (#67, plugin **2.27.0+** / PWA **181+**) | manage_options + visitante | Admin: tipo «Portão (rótulo)», nome PT, posicionar, F5 persiste; PWA Diagrama: texto (sem pin); Guia: portões **fora** do combobox; legenda PWA/PDF numerada **sem** portões; PDF: rótulo no mapa; sem rotas balcão→portão |
| 2z9 | **Mapa — seta balcão→destino no diagrama** (#68, PWA **182+**) | view_ops / visitante | Guia: balcão + destino → Diagrama: seta vermelha recta entre pinos; muda ao trocar seleção; zoom/pan alinhado; só balcão ou só destino → sem seta; offline OK |
